            ...
            if (user) {
                if (descriptSi.length > 1) {
                    ...
                    if (usersblacklist.includes(userID)) {
                        ...
                        message.channel.send(`✅ **Utilisateur <@${userID}> débanni par <@${message.author.id}> !**`)
                        message.client.channels.cache.get('797853971162595339').send(`✅ **Utilisateur <@${userID}> débanni par <@${message.author.id}> !**`)
                    } else {
                        client.users.cache.get(userID).send(new Discord.MessageEmbed()
                        ...
                        message.channel.send(`☢️ **Utilisateur <@${userID}> banni par <@${message.author.id}> !**`)
                        message.client.channels.cache.get('797853971162595339').send(`☢️ **Utilisateur <@${userID}> banni par <@${message.author.id}> !**`)
                    }
                } else {
